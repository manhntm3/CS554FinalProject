...
The code is at \url{https://github.com/manhntm3/CS554FinalProject}.

All memory operations are executed using \textbf{volatile inline PTX assembly} to prevent compiler reordering.
The implementation supports inter-block (different SMs) and intra-block (different warps) configurations.
...
// Plain global store/load (no scope)
st.global.u32 [x], 1;
r0 = ld.global.u32 [y];
\end{lstlisting}
\end{minipage}
...
\textbf{RELAXED GPU}
\begin{lstlisting}[language=C]
// Participates in GPU coherence domain
st.global.relaxed.gpu.u32 [x], 1;
r0 = ld.global.relaxed.gpu.u32 [y];
\end{lstlisting}
\end{minipage}
\caption{SB PTX: plain weak vs.\ GPU-scope relaxed}
\end{figure}
...
\end{lstlisting}
\end{minipage}
\caption{MP PTX: Release/Acquire vs.\ Fence SC (GPU scope)}
\end{figure}
...
All inline PTX instructions survive the compilation pipeline verbatim; no merging or reordering was observed.
...
As a result, P1 could exit the barrier still holding a stale \texttt{flag=1} from iteration $i-1$, then read \texttt{data=0} from P0's reset — a false MP weak outcome.
...
\centering
\small
\caption{MP scalar implementation (first try, 5M iterations) — spurious results}
\label{tab:mp-scalar}
\begin{tabular}{lrr}
\toprule
...
\subsection{Second Try: LB Kernel Bug}

The original LB kernel stored the loaded value (always 0) instead of the constant 1, making the weak outcome \texttt{r0==1 \&\& r1==1} impossible:

\begin{lstlisting}[language=C]
// WRONG (stores v, which is always 0):
int v = 0;
v = ptx_ld_weak(addr_x);  // loads from x (wrong: should be y)
ptx_st_weak(addr_y, v);   // stores 0 (wrong: should be 1 to x)

// CORRECT:
r0 = ptx_ld_weak(addr_y);  // load from y
ptx_st_weak(addr_x, 1);    // store constant 1 to x
\end{lstlisting}

The original result (100\% of outcomes were $(0,0)$) was a vacuous tautology, not a meaningful measurement.

\subsection{Third Try: Fixes Applied}

All experiments in Section~\ref{sec:results} use:
\begin{enumerate}
  \item \textbf{Array-strided layout for all tests} (including MP): each iteration uses a fresh, pre-zeroed memory slot.
  \item \textbf{Correct LB pattern}: P0 reads \texttt{y[i]} and stores 1 to \texttt{x[i]}; P1 reads \texttt{x[i]} and stores 1 to \texttt{y[i]}.
  ...
  \item \textbf{New IRIW test}: four-block test for multi-copy atomicity.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{ACQ/REL (SYS) shows 99.999\% $(0,0)$ outcomes}: the consumer almost always reads both values as 0. This is a timing effect — SYS-scope operations route through the host memory path and are significantly slower, so the consumer consistently reads both locations before the producer completes.
...
\subsection{Hardware Stronger Than Spec}

The PTX memory model is deliberately weak — it specifies the \emph{minimum} ordering guarantee the hardware must provide.
Our results show the Tesla T4 consistently provides \emph{stronger} ordering than required:

\begin{table}[H]
\centering
\small
\caption{PTX spec vs.\ Tesla T4 hardware behavior}
\label{tab:spec-vs-hw}
\begin{tabular}{l l l}
\toprule
\textbf{Test} & \textbf{PTX Spec} & \textbf{T4 Hardware} \\
\midrule
SB RELAXED GPU     & Weak allowed & Weak never observed \\
SB ACQ/REL GPU     & Weak allowed & Weak never observed \\
MP RELAXED GPU     & Weak allowed & Weak never observed \\
LB (all variants)  & Weak allowed & Weak never observed \\
IRIW (all variants)& Weak allowed (WEAK) & Weak never observed \\
\midrule
SB WEAK (inter-block)         & Weak allowed & 0.017\% weak observed \\
SB RELAXED CTA (inter-block)  & Weak allowed & 0.016\% weak observed \\
\bottomrule
\end{tabular}
\end{table}

Only \texttt{WEAK} and cross-block \texttt{RELAXED CTA} produce observable weak behavior.
The common factor: both bypass the L2 cache coherence protocol.
Plain stores (\texttt{.weak}) and CTA-scoped operations between different SMs can remain in the local SM's L1 write buffers without being propagated to L2, creating genuine store-load reordering.

\subsection{The Scalar vs.\ Array Methodology Bug}

The scalar reuse approach represents a common pitfall in GPU litmus testing:
a software barrier is not equivalent to a memory fence at the scope of the test's operations.
Specifically, the PTX memory model requires a \emph{matching pair} of release/acquire operations at the \emph{same scope} to establish happens-before.
A spin on a \texttt{volatile} integer combined with \texttt{\_\_threadfence()} (a non-scoped fence) cannot substitute for this.

This is relevant to the broader community: any GPU litmus test that reuses a scalar and relies on a software barrier is suspect \cite{Sorensen2016InterWorkgroup}.

\subsection{Timing Artifacts in the Outcome Distributions}

The full histograms reveal strong execution-order asymmetries:
\begin{itemize}
  \item \textbf{SB WEAK}: 99.98\% of outcomes are $(0,1)$ (P1 reads x=1 after P0 writes it, but P0 reads y=0 before P1 writes it). This means P0 (block 0) consistently runs slightly ahead of P1 (block 1) at the OS/hardware scheduling level.
  \item \textbf{LB RELAXED GPU}: 81\% $(1,0)$ means P1 tends to lead P0. The asymmetry arises because the two blocks (0 and 1) are mapped to SMs with different workloads or clock phases.
  \item \textbf{FENCE SC GPU}: produces almost exclusively $(1,1)$ for SB — the strong fence forces both operations to be visible before either proceeds, so both threads always see the other's write.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Multi-run statistics confirm reproducibility.}
    With stddev $\approx 0.002\%$ for SB WEAK, the result is clearly reproducible and not noise. All zero-weak results show stddev $= 0.0000\%$ across all 5 runs — these are hard guarantees, not lucky measurements.
\end{enumerate}
